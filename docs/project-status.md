...
| F04 | Candidatures et roster | `IN_PROGRESS` | `feat/applications` / `fix/application-status-visibility` | Création bloquée : intégration GitHub sans permission | 99 fichiers, 255 tests verts ; lint, typecheck et build API/web verts | Autorisation par ressource, anti-doublon, transaction de capacité, origine et validation stricte | [Fiche F04](features/007-applications.md) |
