...
export function createInMemoryDashboardRepository(input: { populated?: boolean; fail?: Array<'nextSession' | 'activeGames' | 'applications' | 'invitations' | 'scheduling' | 'attendance' | 'management'> } = {}): DashboardRepository {
  const shouldFail = (source: string) => input.fail?.includes(source as never)
  const user: DashboardUser = { id: userId, username: 'MJ', avatarUrl: null }
  ...
  const management: GameManagementView = { game, members: [{ gameId, userId, username: 'MJ', avatarUrl: null, role: 'GM', status: 'ACTIVE', joinedAt: now.toISOString() }], applications, invitations: [], nextSession: session, openProposalCount: 1 }
  ...
  return {
    ...
    async getGameManagement(_gameId, ownerId) { if (shouldFail('management')) throw new Error('database management'); return input.populated && ownerId === userId ? management : null },
  }
}
