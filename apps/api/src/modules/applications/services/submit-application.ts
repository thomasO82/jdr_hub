...
export async function submitApplication(input: { userId: string; gameId: string; message?: ApplicationCommand['message']; repository: ApplicationRepository }): Promise<Application> {
  ...
  if (await input.repository.findByGameAndUser(game.id, input.userId)) throw new Error('APPLICATION_CONFLICT')
  return input.repository.create({ gameId: game.id, userId: input.userId, message: input.message ?? null })
}
