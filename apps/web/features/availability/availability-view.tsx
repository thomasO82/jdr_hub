...
export function AvailabilityView({ initial }: { initial: AvailabilitySnapshot | null }) {
  ...
  async function load(): Promise<void> {
    ...
    const snapshot = await createAvailabilityApi().get()
  ...
  if (loading) return <AppShell active="Profil"><main className="min-h-screen px-5 pb-28 pt-24 md:px-8 md:pt-10"><LoadingState label="Chargement de vos disponibilités" /></main></AppShell>
  if (loadingError) return <AppShell active="Profil"><main className="min-h-screen px-5 pb-28 pt-24 md:px-8 md:pt-10"><section className="mx-auto max-w-2xl rounded-xl border border-error/30 bg-surface p-8"><ErrorState message="Réessayez dans un instant pour renseigner vos créneaux." onRetry={() => { void load() }} title="Disponibilités indisponibles" /></section></main></AppShell>
  ...
  return <AppShell active="Profil">
    <main className="min-h-screen bg-background px-5 pb-28 pt-24 font-body text-on-surface md:px-8 md:pb-12 md:pt-10 lg:px-10">
      <section className="mx-auto max-w-6xl" aria-labelledby="availability-title">
        ...
        {error ? <p className="mt-6 rounded-lg border border-error/30 bg-error-container p-3 text-sm text-on-error-container" role="alert">{error}</p> : null}
        {message ? <p className="mt-6 rounded-lg border border-primary/30 bg-primary-fixed p-3 text-sm text-on-primary-fixed" role="status">{message}</p> : null}
        <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(280px,1fr)]">
          <section className="rounded-xl border border-outline-variant/30 bg-surface p-5 shadow-sm" aria-labelledby="weekly-title">
            <div className="flex items-start justify-between gap-4"><div><h2 className="m-0 font-display text-2xl font-semibold" id="weekly-title">Chaque semaine</h2><p className="m-0 mt-1 text-sm text-on-surface-variant">Horaires locaux · {payload.timezone}</p></div><label className="text-right text-xs font-semibold text-on-surface-variant" htmlFor="timezone">Fuseau<select className="mt-1 block min-h-12 rounded-lg border border-outline-variant bg-surface px-2 text-sm text-on-surface focus-visible:outline-2 focus-visible:outline-primary" id="timezone" value={payload.timezone} onChange={(event) => setPayload({ ...payload, timezone: event.target.value })}><option>Europe/Paris</option><option>Europe/London</option><option>America/Montreal</option></select></label></div>
          ...
          <aside className="h-fit rounded-xl border border-outline-variant/30 bg-surface p-5 shadow-sm" aria-labelledby="preferences-title">
            ...
            <label className="mt-5 block text-sm font-semibold" htmlFor="experience">Niveau d’expérience<select className="mt-2 min-h-12 w-full rounded-lg border border-outline-variant bg-surface px-3 text-sm font-normal focus-visible:outline-2 focus-visible:outline-primary" id="experience" value={payload.preferences.experienceLevel ?? ''} onChange={(event) => setPayload({ ...payload, preferences: { ...payload.preferences, experienceLevel: (event.target.value || null) as AvailabilityPayload['preferences']['experienceLevel'] } })}><option value="">À choisir</option><option value="BEGINNER">Débutant</option><option value="INTERMEDIATE">Intermédiaire</option><option value="VETERAN">Vétéran</option></select></label>
            <button className="mt-8 inline-flex min-h-12 w-full items-center justify-center rounded-lg bg-primary px-4 text-sm font-semibold text-on-primary transition-colors hover:bg-primary-container focus-visible:outline-2 focus-visible:outline-primary motion-reduce:transition-none disabled:cursor-not-allowed disabled:opacity-60" disabled={saving} onClick={() => void save()} type="button">{saving ? 'Enregistrement…' : 'Enregistrer'}</button>
          </aside>
        </div>
      </section>
    </main>
