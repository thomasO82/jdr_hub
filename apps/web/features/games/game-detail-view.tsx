    ...
    <AppShell>
      <main className="min-h-screen bg-background px-5 py-6 font-body text-on-surface md:px-6 md:py-10 lg:px-10">
        <section className="mx-auto max-w-6xl" aria-labelledby="game-detail-title">
          ...
          <header className="flex min-h-[20rem] items-end justify-between gap-6 rounded-3xl bg-gradient-to-br from-cover-night via-connection-mid to-cover-violet p-8 text-white shadow-lg shadow-primary/10 max-md:min-h-[17rem] max-md:flex-col max-md:items-start max-md:p-6">
            <div>
            ...
            <div className="flex items-center gap-2.5 rounded-full border border-white/35 bg-black/25 px-3.5 py-2.5 max-md:w-full max-md:justify-center">
              ...
              <span><small className="block font-label text-[0.65rem] uppercase text-violet-200">Maître du jeu</small><strong className="block font-body text-sm">Organisateur de la partie</strong></span>
            </div>
          </header>
          <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_22.5rem]">
            <article className="rounded-2xl border border-surface-container-highest bg-surface p-8 shadow-sm max-md:p-6">
              ...
              <ul className="m-0 flex flex-wrap gap-2 p-0 [list-style:none]">{game.tags.map((tag) => <li className="rounded-full bg-primary-fixed px-2 py-1 font-body text-xs font-semibold text-on-primary-fixed" key={tag}>#{tag}</li>)}</ul>
            </article>
