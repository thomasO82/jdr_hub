    ...
    <article className="overflow-hidden rounded-2xl border border-surface-container-highest bg-surface shadow-sm transition-transform hover:-translate-y-0.5 hover:shadow-lg hover:shadow-primary/10">
      <Link className="block text-on-surface no-underline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary" href={`/parties/${game.slug}`}>
        <div className={`relative flex min-h-48 items-end justify-between overflow-hidden px-3.5 py-3.5 text-white ${cover}`}>
          <span className="relative inline-flex items-center gap-1 rounded-md bg-black/40 px-2 py-1.5 font-label text-[0.7rem] font-semibold"><UsersRound aria-hidden="true" size={14} />{game.maxPlayers} places</span>
          <span className="relative rounded-md bg-primary px-2 py-1.5 font-label text-[0.7rem] font-semibold uppercase">{game.system}</span>
        </div>
        <div className="grid gap-2.5 p-4">
          <p className="m-0 font-label text-[0.65rem] font-bold uppercase tracking-wider text-primary">{game.type === 'CAMPAIGN' ? 'Campagne' : 'One-shot'}</p>
          ...
          <div className="flex justify-between gap-2 border-t border-surface-container pt-3 font-label text-[0.68rem] font-semibold text-on-surface-variant">
          ...
          <ul className="m-0 flex flex-wrap gap-2 p-0 [list-style:none]">
  ...
  return (
    <AppShell>
      <main className="min-h-screen bg-background px-5 py-6 font-body text-on-surface md:px-6 md:py-10 lg:px-10">
        <form className="mx-auto grid max-w-6xl items-start gap-6 lg:grid-cols-[18rem_minmax(0,1fr)]" action="/parties" method="get">
          <aside className="rounded-xl border border-surface-container-highest bg-surface p-4 shadow-sm" aria-label="Filtres des parties">
            ...
            <FiltersToggle>
              ...
              <fieldset className="grid gap-2.5 border-0 p-0"><legend className="mb-1 font-label text-xs font-bold uppercase tracking-wider text-on-surface-variant">Format</legend><div className="grid grid-cols-2 gap-2"><label className="grid min-h-8 cursor-pointer place-items-center rounded-lg border border-primary-fixed-dim font-body text-xs text-on-surface-variant has-[:checked]:bg-primary-fixed has-[:checked]:font-semibold has-[:checked]:text-primary"><input className="sr-only" type="radio" name="visualFormat" value="ONLINE" defaultChecked /><span>En ligne</span></label><label className="grid min-h-8 cursor-pointer place-items-center rounded-lg border border-surface-container-highest font-body text-xs text-on-surface-variant has-[:checked]:bg-primary-fixed has-[:checked]:font-semibold has-[:checked]:text-primary"><input className="sr-only" type="radio" name="visualFormat" value="TABLE" /><span>Sur table</span></label></div></fieldset>
          ...
          <section className="min-w-0" aria-labelledby="games-title">
